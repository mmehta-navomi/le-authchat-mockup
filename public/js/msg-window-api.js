...
function prepareToConnect() {
  console.log('prepareToConnect')
  // $('#connect').text('connect').unbind('click').click(() => {
    $('#connect').text('connecting...');
    // const account = $('#account').prop('disabled',false).val();
    const account = '75070123';
...
function handleOpenedSocket(socket,jwt) {
  ...
  // $('#connect').text('socked closed. Reload page to open').unbind('click').click(() => socket.ws.close());
...
function handleConversationNotification(socket,notificationBody,openConvs) {
  notificationBody.changes.forEach(change => {
    if (change.type === 'UPSERT') {
      ...
      var cobrowse_channel_obj = _.find(change.result.conversationDetails.dialogs,['channelType', 'COBROWSE']);
      //get metadata info
      if(cobrowse_channel_obj){
        var agent_id = _.find(cobrowse_channel_obj.participantsDetails,['role','ASSIGNED_AGENT']);
        console.log("CoBrowse Video Call Offered")
        lpTag.events.publish("lpCoBrowse", "cobrowseOffered", {
           	serviceId: cobrowse_channel_obj.metaData.serviceId,
           	agentId: agent_id.id,
           	visitorName: "Testing Visitor",
           	mode: cobrowse_channel_obj.metaData.mode,
           	ssid: lpTag.taglets.lp_monitoringSDK.getSid(),
           	svid: lpTag.taglets.lp_monitoringSDK.getVid(),
           }
          );
        console.log("CoBrowse Video Call Accepted")
...
const apiRequestTypes = ['cqm.ExConversationChangeNotification','cqm.SubscribeExConversations','ms.PublishEvent','cm.ConsumerRequestConversation','ms.SubscribeMessagingEvents','InitConnection','cm.UpdateConversationField'];
